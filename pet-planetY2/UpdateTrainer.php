...
if (isset($_GET['id'])) {
    ...
    $trainerData = $trainer->fetchTrainerDataFromDatabaseByUserId($userId);
...
if (isset($_SESSION['userData']) && isset($_SESSION['trainerData'])) {
...
if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST) {
    ...
    $sessionAddress = $trainerAddress->address ?? "";
    ...
    if (empty($validation->getErrors())) {
        ...
        $trainer->setWork_days($_POST['work_days'])
        ...
        $address = new Address;
        $address->setId($address_id)->setAddress(isset($_POST['address']) ? $_POST['address'] : $trainerAddress->address);
        if ($user->update() && $trainer->update() && $address->updateById($address_id, isset($_POST['address']) ? $_POST['address'] : $trainerAddress->address)) {
            ...
            header('refresh:3;url=TrainerProfile.php');
        }
    } else {
                ...
                <div class="left">
                    ...
                    <form method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="field2c" class="form-control" name="address" value="<?= isset($_POST['address']) ? $_POST['address'] : ($trainerAddress && isset($trainerAddress->address) ? $trainerAddress->address : '') ?>">
                            <?= $validation->getMessage('address') ?>
                        </div>
                        <br><br>
                        <button type="submit" name="update-image" class="btn btn-primary btn-lg">Update</button>
                        <button type="button" class="btn btn-primary btn-lg"><a href="TrainerProfile.php">Cancel</a></button>
                    </form>
                </div>
            </div>
            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-6">
