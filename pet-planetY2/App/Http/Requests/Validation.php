...
class Validation
{
    ...
    public function validateAddress(string $address): self
    {
        $apiKey = "00Fym3GhTRmEXjth4EMBSuDeUbLjnqG8";
        $geocodingURL = "https://www.mapquestapi.com/geocoding/v1/address";
        $requestURL = $geocodingURL . "?key=" . $apiKey . "&location=" . urlencode($address);

        $response = file_get_contents($requestURL);
        $data = json_decode($response);
        $latLng = $data->results[0]->locations[0]->latLng;
        $latitudeInput = $latLng->lat;
        $longitudeInput = $latLng->lng;

        if (($latitudeInput == 0 && $longitudeInput == 0) || ($latitudeInput <= 0) || ($longitudeInput <= 0)) {
            $this->errors[$this->inputValueName][__FUNCTION__] = "Invalid address or geocoding error.";
        }
        return $this;
    }
